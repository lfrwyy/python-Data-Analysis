...
def ekf_predict(X):
    # 扩展卡尔曼滤波实现
    def f(x):
        return x  # 状态转移函数（根据具体系统模型修改）

    def jacobian_f(x):
        return np.eye(len(x))  # 雅可比矩阵（根据模型调整）

    n_dim = X.shape[1]
    ekf = KalmanFilter(
        transition_functions=f,
        observation_functions=lambda x: x,
        transition_covariance=0.1 * np.eye(n_dim),
        observation_covariance=0.5 * np.eye(n_dim),
        initial_state_mean=X.iloc[0].values.astype(float),
        initial_state_covariance=np.eye(n_dim)
    ...
    state_means, _ = ekf.smooth(X.values.astype(float))
    return pd.DataFrame(state_means,
                        columns=[f'ekf_{col}' for col in X.columns],
                        index=X.index)
...
def main():
    ...
    result.to_csv('e+l预测结果.csv', index=False, encoding='GB2312')
    ...
    # 使用模型预测新数据
    predict_new_data('10finaldata.csv', scaler, level_clf, '10EKF+BiLSTM_new_predictions.csv')
